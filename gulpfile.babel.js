...
// Css
import { cssTask, cssTaskDev, cssClean } from './gulp/scss_tasks';
...
// SCSS - dev folder
gulp.task('clean-css-dev', async () => {
  cssClean('./public/css/*.css', ['./public/*.html'], './public/css');
});
// SCSS - docs folder
gulp.task('clean-css-build', async () => {
  cssClean('./public/css/*.css', ['./public/*.html'], './docs/css');
});
...
gulp.task('dev', () => {
  ...
  gulp
    .watch('./src/views/**/*.pug', gulp.series('pug-dev'))
    .on('change', reload);
